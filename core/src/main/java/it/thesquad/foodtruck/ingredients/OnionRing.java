...
public class OnionRing implements Ingredient {
    ...
    public OnionRing(Sprite Onion) {
        this.onion = Onion;
        this.cookedPercentage = 0;
    }

    public OnionRing(Sprite Onion, int cookedPercentage) {
        this.onion = Onion;
    ...
    public void draw(SpriteBatch batch) {
        if (onion != null) {
            ...
            onion.setX(Player.getInstance().getX() + (Player.getInstance().getTexture().getWidth() / 2f) - (onion.getTexture().getWidth() / 2f));
            onion.setY(Player.getInstance().getY() + (Player.getInstance().getTexture().getHeight() / 2f) - (onion.getTexture().getHeight() / 2f) - 67);
    ...
    public void setRing(Sprite Onion) {
        this.onion = Onion;
    }
