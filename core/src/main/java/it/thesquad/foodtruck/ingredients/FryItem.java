package it.thesquad.foodtruck.ingredients;

import com.badlogic.gdx.graphics.Color;
import com.badlogic.gdx.graphics.g2d.SpriteBatch;
import it.thesquad.foodtruck.logic.Sprite;
import it.thesquad.foodtruck.player.Player;

public class FryItem implements Ingredient {

    private Sprite FryItem;
    private int cookedPercentage;

    private final Color rawFryItemColor = new Color(219/255f, 108/255f, 141/255f, 1f); // Raw FryItem color (219, 108, 141)

    public FryItem(Sprite FryItem) {
        this.FryItem = FryItem;
        this.cookedPercentage = 0;
    }

    public FryItem(Sprite FryItem, int cookedPercentage) {
        this.FryItem = FryItem;
        this.cookedPercentage = cookedPercentage;
    }

    /**
     * Calculates the overlay color for the FryItem based on its cooked percentage.
     * Transitions from rawFryItemColor to white (cooked), then to black (burnt).
     *
     * @return The calculated tint color.
     */
    private Color getOverlayColor() {
        Color tint;
        float currentCookedPercentage = this.cookedPercentage; // Use the field

        if (currentCookedPercentage < 50f) {
            float progress = currentCookedPercentage / 50f;
            tint = new Color(rawFryItemColor).lerp(Color.WHITE, progress);
        } else {
            float progress = Math.min((currentCookedPercentage - 50f) / 50f, 1.0f);
            tint = new Color(Color.WHITE).lerp(Color.BLACK, progress);
        }
        return tint;
    }

    @Override
    public void draw(SpriteBatch batch) {
        if (FryItem != null) {
            FryItem.setX(Player.getInstance().getX() + (Player.getInstance().getTexture().getWidth() / 2f) - (FryItem.getTexture().getWidth() / 2f));
            FryItem.setY(Player.getInstance().getY() + (Player.getInstance().getTexture().getHeight() / 2f) - (FryItem.getTexture().getHeight() / 2f) - 67);

            batch.setColor(getOverlayColor());
            FryItem.render(batch);
            batch.setColor(Color.WHITE);
        }
    }

    public Sprite getFryItem() {
        return FryItem;
    }

    public void setFryItem(Sprite FryItem) {
        this.FryItem = FryItem;
    }

    public int getCookedPercentage() {
        return cookedPercentage;
    }

    public void setCookedPercentage(int cookedPercentage) {
        if (cookedPercentage < 0) {
            cookedPercentage = 0;
        }
        if (cookedPercentage > 100) {
            cookedPercentage = 100;
        }
        this.cookedPercentage = cookedPercentage;
    }
}
